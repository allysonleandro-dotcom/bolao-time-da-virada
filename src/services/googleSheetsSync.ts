...
export async function fetchPixDataFromSheets(spreadsheetUrlOrId?: string): Promise<{
  ...
  try {
    ...
    if (!res.ok) {
      throw new Error(`Erro HTTP ${res.status} ao buscar dados de PIX`);
    }
    const data = await res.json();
    return {
      success: true,
    ...
    console.error('Erro ao buscar dados de PIX:', err);
    return {
      success: false,
      registroPix: [],
      historicoPix: [],
      pixPessoal: [],
      tabs: [],
      error: err?.message || 'Falha ao conectar com o serviço de PIX da planilha.',
    };
  }
}
