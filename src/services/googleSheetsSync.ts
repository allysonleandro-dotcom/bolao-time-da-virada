...
export function processIndependenciaTab(
  ...
  b1ParticipantsList.forEach((item) => {
    ...
    const parsedVal = parseMoneyValue(item.rawVal, item.isPaid ? 60.0 : 0.0);
    const itemPaid = item.isPaid ? (parsedVal > 0 ? parsedVal : 60.0) : 0.0;
  ...
  b2ParticipantsList.forEach((item) => {
    const norm = normalizeName(item.name);
    const pId = participantIdByNameMap.get(norm) || nameToId(item.name);
    const parsedVal = parseMoneyValue(item.rawVal, item.isPaid ? 39.0 : 0.0);
    const itemPaid = item.isPaid ? (parsedVal > 0 ? parsedVal : 39.0) : 0.0;
