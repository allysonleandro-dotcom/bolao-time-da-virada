...
export function processIndependenciaTab(
  ...
  b1ParticipantsList.forEach((item) => {
    ...
    const parsedVal = parseMoneyValue(item.rawVal, item.isPaid ? quotaPriceB1 : 0.0);
    const itemPaid = item.isPaid ? (parsedVal > 0 ? parsedVal : quotaPriceB1) : 0.0;
  ...
  b2ParticipantsList.forEach((item) => {
    const norm = normalizeName(item.name);
    const pId = participantIdByNameMap.get(norm) || nameToId(item.name);
    const parsedVal = parseMoneyValue(item.rawVal, item.isPaid ? quotaPriceB2 : 0.0);
    const itemPaid = item.isPaid ? (parsedVal > 0 ? parsedVal : quotaPriceB2) : 0.0;
