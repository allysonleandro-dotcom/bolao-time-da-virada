...
export function processBoloesRegularesTab(
  ...
  // 2. Localiza cabeçalho da tabela de cotas / participantes
  let headerIndex = -1;
  let colCota = -1;
  let colName = -1;
  let colValor = -1;
  ...
  for (let r = 0; r < Math.min(15, rows.length); r++) {
    ...
    const hasValor = row.some((c) => /valor|pago|r\$/i.test(c));
    const hasStatus = row.some((c) => /status/i.test(c));

    if ((hasCota && hasName) || (hasName && (hasValor || hasStatus))) {
      headerIndex = r;
      row.forEach((cell, idx) => {
        ...
        else if ((/VALOR/i.test(u) || /R\$/i.test(u)) && colValor === -1) colValor = idx;
  ...
  // A leitura da linha de participantes em BOLÕES_REGULARES deve ser feita estritamente a partir da LINHA 7.
  // Em matriz originada da célula A1 (onde rows[0] é a Linha 1):
  // Linha 1 = rows[0], Linha 2 = rows[1], Linha 3 = rows[2], Linha 4 = rows[3], Linha 5 = rows[4],
  // Linha 6 = rows[5] (Cabeçalho da tabela: COTA, NOME, VALOR PAGO, STATUS),
  // Linha 7 = rows[6] (Primeiro participante: Cota 1, ANA C S DE OLIVEIRA, R$ 25,00, TRUE).
  let startRow = 6;
  if (rows.length >= 7 && (rows[0].length === 0 || headerIndex >= 4)) {
    startRow = 6; // Linha 7 da planilha (base 0 = índice 6)
