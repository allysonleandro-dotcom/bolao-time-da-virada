...
export function processBoloesRegularesTab(
  ...
  for (let r = startRow; r < rows.length; r++) {
    ...
    if (totalQuotas > 0 && cotaNum > totalQuotas && explicitTotalQuotasFound) {
      // Ignora linhas extras de modelo além das cotas totais oficiais
      break;
    }
  ...
  // Ajusta totalQuotas se não tiver sido explícito
  if (!explicitTotalQuotasFound) {
    if (totalBolao > 0 && quotaPrice > 0) {
      totalQuotas = Math.round(totalBolao / quotaPrice);
    } else {
      totalQuotas = Math.max(maxCotaFound, 4);
    }
  }
  if (!totalBolao || totalBolao <= 0) {
