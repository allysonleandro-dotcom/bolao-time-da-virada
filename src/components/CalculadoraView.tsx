...
export const CalculadoraView: React.FC<CalculadoraViewProps> = ({
  ...
  const [gameMix, setGameMix] = useState<GameMixItem[]>([
    { id: '1', lotteryType: 'mega-sena', numbersCount: 11, quantity: 1 }, // 2.772,00
    { id: '2', lotteryType: 'mega-sena', numbersCount: 6, quantity: 14 },  // 84,00 (14 apostas de 6 dezenas = total 2.856,00)
  ]);
  const [targetQuotas, setTargetQuotas] = useState(50);
  ...
  const handleLoadSpreadsheetPreset = (
    ...
    if (presetType === 'virada_57') {
      setSelectedLottery('mega-sena');
      setGameMix([
        { id: '1', lotteryType: 'mega-sena', numbersCount: 11, quantity: 2 }, // 2 apostas de 11 dezenas (2 x 2.772,00 = R$ 5.544,00)
      ]);
      setTargetQuotas(100); // 100 cotas a R$ 60,00 = R$ 6.000,00
      setAdminFeePercent(0);
      setExtraCost(456);
      setExtraCostInput('456,00');
      setReserveFund(456);
    } else if (presetType === 'virada_35') {
      setSelectedLottery('mega-sena');
      setGameMix([
        { id: '1', lotteryType: 'mega-sena', numbersCount: 13, quantity: 1 }, // 1 aposta de 13 dezenas (R$ 10.296,00)
      ]);
      setTargetQuotas(100); // 100 cotas a R$ 113,00 = R$ 11.300,00
      setAdminFeePercent(0);
      setExtraCost(1004);
      setExtraCostInput('1004,00');
      setReserveFund(1004);
    } else if (presetType === 'virada_13d') {
      setSelectedLottery('mega-sena');
      setGameMix([
        { id: '1', lotteryType: 'mega-sena', numbersCount: 13, quantity: 1 }, // 10.296,00 (1 aposta de 13 dezenas oficial Caixa)
      ]);
      setTargetQuotas(100); // 10.296,00 / 100 cotas = R$ 102,96 por cota
      setAdminFeePercent(0);
      setExtraCost(0);
      setExtraCostInput('0,00');
      setReserveFund(0);
    } else if (presetType === 'regular_25') {
      setSelectedLottery('mega-sena');
      setGameMix([
  ...
  // Centavos precision (2 decimal places: ex. R$ 57,12, R$ 35,70)
  const roundedQuotaPrice = Math.ceil(rawQuotaPrice * 100) / 100;
  const totalRevenue = Math.round(targetQuotas * roundedQuotaPrice * 100) / 100;
  const netBalance = Math.round((totalRevenue - (totalGamesCost + extraCost + reserveFund)) * 100) / 100;
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <div className="bg-emerald-950 text-white rounded-3xl border border-emerald-800 p-6 sm:p-7 shadow-md space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2.5 pt-1">
                <button
                  type="button"
                  onClick={() => handleLoadSpreadsheetPreset('virada_57')}
                  className="p-3 rounded-2xl bg-emerald-700/80 hover:bg-emerald-600 border border-emerald-400/50 hover:border-emerald-300 text-left transition group active:scale-95 flex flex-col justify-between shadow-xs"
                >
                  <div>
                    <div className="text-[10px] font-black text-amber-300 group-hover:text-amber-200 uppercase flex items-center gap-1">
                      <span>🇧🇷 Mega da Virada #1</span>
                    </div>
                    <div className="font-black text-xs text-white mt-0.5">
                      2x 11 Dezenas (924 jogos eq.)
                    </div>
                  </div>
                  <div className="mt-2">
                    <div className="text-sm font-black text-white group-hover:text-amber-200">
                      R$ 60,00 <span className="text-[10px] font-normal text-emerald-200">/ 100 cotas</span>
                    </div>
                    <div className="text-[10px] text-emerald-200 font-medium">
                      Apostas: R$ 5.544,00 | Total: R$ 6.000
                    </div>
                  </div>
                </button>
                ...
                <button
                  type="button"
                  onClick={() => handleLoadSpreadsheetPreset('virada_35')}
                  className="p-3 rounded-2xl bg-teal-800/80 hover:bg-teal-600 border border-teal-400/50 hover:border-teal-300 text-left transition group active:scale-95 flex flex-col justify-between shadow-xs"
                >
                  <div>
                    <div className="text-[10px] font-black text-amber-300 group-hover:text-amber-200 uppercase flex items-center gap-1">
                      <span>🇧🇷 Mega da Virada #2</span>
                    </div>
                    <div className="font-black text-xs text-white mt-0.5">
                      1x 13 Dezenas (1.716 jogos eq.)
                    </div>
                  </div>
                  <div className="mt-2">
                    <div className="text-sm font-black text-white group-hover:text-amber-200">
                      R$ 113,00 <span className="text-[10px] font-normal text-teal-200">/ 100 cotas</span>
                    </div>
                    <div className="text-[10px] text-teal-200 font-medium">
                      Aposta: R$ 10.296,00 | Total: R$ 11.300
                    </div>
                  </div>
                </button>
                ...
                <button
                  type="button"
                  onClick={() => handleLoadSpreadsheetPreset('virada_13d')}
                ...
                >
                  <div>
                    ...
                    <div className="font-black text-xs text-white mt-0.5">
                      1x 13 Dezenas (1.716 eq.)
                    </div>
                  </div>
                  <div className="mt-2">
                    <div className="text-sm font-black text-emerald-300 group-hover:text-white">
                      R$ 102,96 <span className="text-[10px] font-normal text-slate-300">/ 100 cotas</span>
                    </div>
              ...
              <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-emerald-900/60 text-[11px]">
                <span className="text-emerald-400 font-bold">Mais Modelos:</span>
                <button
                  type="button"
          ...
          <div className="bg-white rounded-3xl border border-slate-200 p-6 sm:p-8 shadow-xs space-y-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="p-5 rounded-2xl border-2 border-emerald-500 bg-emerald-50/40 space-y-3 relative shadow-xs">
                ...
                <div>
                  <h4 className="font-black text-slate-900 text-sm">BOLÃO 1 - MEGA DA VIRADA</h4>
                  <div className="text-2xl font-black text-emerald-700 mt-1">
                    R$ 60,00 <span className="text-xs text-slate-500 font-medium">/ cota</span>
                  </div>
                </div>
                <div className="text-xs text-slate-600 space-y-1 font-medium pt-2 border-t border-emerald-200/70">
                  <div className="flex justify-between">
                    <span>Arrecadação Total:</span>
                    <strong className="text-slate-900">R$ 6.000,00</strong>
                  </div>
                  <div className="flex justify-between text-[11px] text-slate-500">
                    <span>Apostas (2x 11 Dezenas):</span>
                    <strong className="text-emerald-700 font-bold">R$ 5.544,00</strong>
                  </div>
                  <div className="flex justify-between text-[11px] text-slate-500">
                    <span>Fundo de Reserva / Operacional:</span>
                    <span>R$ 456,00</span>
                  </div>
                </div>
                <button
                  onClick={() => {
                    handleLoadSpreadsheetPreset('virada_57');
                    setActiveSubTab('simulador');
                  }}
                  className="w-full text-xs font-black text-white bg-emerald-600 hover:bg-emerald-700 py-2.5 rounded-xl transition text-center mt-2 block active:scale-95 shadow-xs cursor-pointer"
                >
                  Simular este Bolão (R$ 60,00)
                </button>
              </div>

              {/* Bolão 2 Mega da Virada */}
              <div className="p-5 rounded-2xl border-2 border-teal-500 bg-teal-50/40 space-y-3 relative shadow-xs">
                <div className="flex justify-between items-start">
                  <span className="text-[10px] font-black uppercase px-2.5 py-1 rounded-full bg-teal-600 text-white shadow-xs">
                    🇧🇷 Mega da Virada
                  </span>
                  <span className="text-xs font-black text-teal-800 bg-teal-100 px-2 py-0.5 rounded-lg">
                    100 Cotas
                  </span>
                </div>
                <div>
                  <h4 className="font-black text-slate-900 text-sm">BOLÃO 2 - MEGA DA VIRADA</h4>
                  <div className="text-2xl font-black text-teal-700 mt-1">
                    R$ 113,00 <span className="text-xs text-slate-500 font-medium">/ cota</span>
                  </div>
                </div>
                <div className="text-xs text-slate-600 space-y-1 font-medium pt-2 border-t border-teal-200/70">
                  <div className="flex justify-between">
                    <span>Arrecadação Total:</span>
                    <strong className="text-slate-900">R$ 11.300,00</strong>
                  </div>
                  <div className="flex justify-between text-[11px] text-slate-500">
                    <span>Aposta (1x 13 Dezenas):</span>
                    <strong className="text-teal-700 font-bold">R$ 10.296,00</strong>
                  </div>
                  <div className="flex justify-between text-[11px] text-slate-500">
                    <span>Fundo de Reserva / Operacional:</span>
                    <span>R$ 1.004,00</span>
                  </div>
                </div>
                <button
                  onClick={() => {
                    handleLoadSpreadsheetPreset('virada_35');
                    setActiveSubTab('simulador');
                  }}
                  className="w-full text-xs font-black text-white bg-teal-600 hover:bg-teal-700 py-2.5 rounded-xl transition text-center mt-2 block active:scale-95 shadow-xs cursor-pointer"
                >
                  Simular este Bolão (R$ 113,00)
                </button>
              </div>

              {/* Mega Especial 13 Dezenas */}
              <div className="p-5 rounded-2xl border border-slate-200 bg-slate-50/70 space-y-3">
                <div className="flex justify-between items-start">
                  ...
                  <span className="text-xs font-black text-slate-700">100 Cotas</span>
                </div>
                <div>
                  <h4 className="font-black text-slate-900 text-sm">MEGA ESPECIAL (13 DEZENAS)</h4>
                  <div className="text-2xl font-black text-amber-700 mt-1">
                    R$ 102,96 <span className="text-xs text-slate-500 font-medium">/ cota</span>
                  </div>
                </div>
                <div className="text-xs text-slate-600 space-y-1 font-medium pt-2 border-t border-slate-200">
                  <div className="flex justify-between">
                    <span>Arrecadação Total:</span>
                    <strong className="text-slate-900">R$ 10.296,00</strong>
                  </div>
                  <div className="flex justify-between text-[11px] text-slate-500">
                    <span>Custo do Jogo (13D Oficial):</span>
                    <span>R$ 10.296,00</span>
                  </div>
                  <div className="flex justify-between text-[11px] text-slate-500">
                    <span>Equivalência Simples:</span>
                    <span>1.716 apostas</span>
                  </div>
                </div>
                <button
                  onClick={() => {
                    handleLoadSpreadsheetPreset('virada_13d');
                    setActiveSubTab('simulador');
                  }}
                  className="w-full text-xs font-bold text-amber-900 bg-amber-200 hover:bg-amber-300 py-2.5 rounded-xl transition text-center mt-2 block active:scale-95 cursor-pointer"
                >
                  Simular este Bolão (R$ 102,96)
                </button>
              </div>
            </div>

            {/* Modelos Anteriores / Outras Loterias */}
            <div className="pt-3 border-t border-slate-200 flex flex-wrap items-center justify-between gap-3 text-xs">
              <span className="font-bold text-slate-500">Histórico de Modelos Anteriores:</span>
              <div className="flex flex-wrap gap-2">
                <button
                  ...
                  className="px-3 py-1.5 rounded-lg bg-purple-50 hover:bg-purple-100 text-purple-700 font-bold border border-purple-200 transition"
                >
                  Lotofácil 18D (R$ 60,00)
                </button>
                <button
                  ...
                  className="px-3 py-1.5 rounded-lg bg-indigo-50 hover:bg-indigo-100 text-indigo-700 font-bold border border-indigo-200 transition"
                >
                  Lotofácil 3x 17D (R$ 39,00)
                </button>
                <button
                  ...
                  className="px-3 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold border border-slate-200 transition"
                >
                  Mega-Sena Regular (R$ 25,00)
                </button>
              </div>
            </div>
          </div>
