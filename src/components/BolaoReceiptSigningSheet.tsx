...
export const BolaoReceiptSigningSheet: React.FC<BolaoReceiptSigningSheetProps> = ({
  ...
  if (groupByQuota && totalQuotasNum > 0) {
    const quotaToParticipant = new Map<number, { name: string; phone: string }>();
    ...
    bolao.participants.forEach((bp) => {
      ...
      if (bp.quotaNumbers && bp.quotaNumbers.length > 0) {
        bp.quotaNumbers.forEach((qn) => {
          const num = Number(qn);
          if (!isNaN(num)) {
            quotaToParticipant.set(num, { name: cleanName, phone });
          }
        });
      }
    });
    ...
    sortedParticipants.forEach((bp) => {
      ...
      const val = bp.quotas * bolao.quotaPrice;
      ...
      <div className="no-print bg-white rounded-2xl border border-slate-200 p-5 sm:p-6 shadow-sm">
        ...
        <div className="mt-4 pt-4 border-t border-slate-100 flex flex-wrap items-center justify-between gap-3 text-xs">
          <div className="flex flex-wrap items-center gap-4">
            ...
            {groupByQuota && (
              <label className="flex items-center gap-2 cursor-pointer select-none font-medium text-slate-600 ml-2 border-l border-slate-200 pl-3">
                ...
                />
                <span>Incluir cotas disponíveis para preenchimento manual</span>
              </label>
            )}
          </div>
          ...
          <div className="text-slate-400 font-medium text-[11px] flex items-center gap-1">
            ...
            <span>O documento é gerado no formato padrão A4 com linhas para assinatura física.</span>
          </div>
        </div>
            ...
            <table className="w-full border-collapse text-left text-xs border border-slate-300">
              <thead>
                <tr className="bg-slate-900 text-white font-bold text-[11px]">
                  ...
                  <th className="py-2.5 px-3 border border-slate-700 text-center w-36">
                    Contato / Telefone
                  </th>
                  <th className="py-2.5 px-3 border border-slate-700 text-right w-24">
                    Valor
                  </th>
                  <th className="py-2.5 px-3 border border-slate-700 text-center w-52 sm:w-64">
              ...
              <tbody className="divide-y divide-slate-200">
                {rows.map((row, idx) => (
                  ...
                  >
                    ...
                    <td className="py-2 px-3 border border-slate-200 text-center text-slate-600 whitespace-nowrap">
                      {row.phone}
                    </td>
                    <td className="py-2 px-3 border border-slate-200 text-right font-semibold text-slate-800 whitespace-nowrap">
                      {formatCurrency(row.val)}
                    </td>
                    <td className="py-2 px-3 border border-slate-200 relative align-bottom pb-2">
                      {/* Clean line for handwriting signature */}
