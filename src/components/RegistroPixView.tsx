...
export const RegistroPixView: React.FC<RegistroPixViewProps> = ({
  ...
  // Registros adicionados localmente (persistem na sessão caso o backend não esteja disponível)
  const [localCustomRecords, setLocalCustomRecords] = useState<PixTransactionRecord[]>(() => {
    try {
      const saved = localStorage.getItem('local_pix_records_cache');
  ...
  // Transações sintetizadas a partir dos bolões e participantes ativos (caso a aba de PIX na planilha ainda esteja vazia ou em deploy estático como Vercel)
  const synthesizedRecordsFromBoloes = useMemo(() => {
    const list: PixTransactionRecord[] = [];
    const partMap = new Map<string, Participant>();
    participants.forEach((p) => {
      partMap.set(p.id, p);
      if (p.name) partMap.set(p.name.trim().toUpperCase(), p);
    });

    boloes.forEach((b) => {
      b.participants.forEach((bp, idx) => {
        const pObj = partMap.get(bp.participantId) || partMap.get((bp.name || '').trim().toUpperCase());
        const name = (bp.name || pObj?.name || 'Participante').trim();
        if (!name || name.length < 2) return;

        const isPaid = bp.status === 'pago';
        const valor = bp.amountPaid > 0 ? bp.amountPaid : (bp.quotas || 1) * b.quotaPrice;
        const dateStr = b.updatedAt
          ? new Date(b.updatedAt).toLocaleDateString('pt-BR')
          : new Date().toLocaleDateString('pt-BR');

        list.push({
          id: `bolao-tx-${b.id}-${bp.participantId || idx}`,
          dataHora: dateStr,
          nome: name.toUpperCase(),
          valor: valor || b.quotaPrice || 0,
          valorOriginalText: `R$ ${(valor || b.quotaPrice || 0).toFixed(2).replace('.', ',')}`,
          statusPagamento: isPaid,
          statusEnvio: isPaid,
          bolao: b.title,
          observacao: isPaid ? 'Pagamento confirmado' : 'Aguardando comprovante PIX',
  ...
  const loadData = async (showSpinner = true) => {
    ...
    try {
      ...
      // Se não encontrou abas ou se está no Vercel (onde /api retorna 404),
      // o fallback sintetizado dos bolões assume naturalmente sem gerar erro!
    } catch (err: any) {
      console.warn('Sincronização de PIX:', err);
    } finally {
      setIsLoading(false);
  ...
  const currentRecords = useMemo(() => {
    let baseList: PixTransactionRecord[] = [];
    if (activeTab === 'registro_pix') {
      baseList =
        parsedRegistroPix.records.length > 0
          ? parsedRegistroPix.records
          : synthesizedRecordsFromBoloes;
    } else if (activeTab === 'historico_pix') {
      baseList =
        parsedHistoricoPix.length > 0
          ? parsedHistoricoPix
          : synthesizedRecordsFromBoloes.filter((r) => r.statusPagamento);
    } else {
      baseList =
        parsedPixPessoal.length > 0
          ? parsedPixPessoal
          : synthesizedRecordsFromBoloes;
    }

    // Mescla com registros locais inseridos manualmente
    const matchingLocal = localCustomRecords.filter((lr) => {
      if (activeTab === 'registro_pix') return true;
      if (activeTab === 'historico_pix') return lr.statusPagamento;
      return true;
    });

    return [...matchingLocal, ...baseList];
  }, [
    activeTab,
    parsedRegistroPix,
    parsedHistoricoPix,
  ...
  const stats = useMemo(() => {
    ...
    const summaryCaixa =
      parsedRegistroPix.summary.caixa && parsedRegistroPix.summary.caixa > 0
        ? parsedRegistroPix.summary.caixa
        : paidValue;
    ...
    return {
      ...
      pendingCount,
      summary: {
        ...parsedRegistroPix.summary,
        caixa: summaryCaixa,
        totalArrecadado: parsedRegistroPix.summary.totalArrecadado || paidValue,
        totalRegistros: totalRecords,
        totalConfirmados: paidRecords,
      },
    };
  ...
  const handleCreatePixRecord = async (e: React.FormEvent) => {
    ...
    const newRecordItem: PixTransactionRecord = {
      id: `pix-local-${Date.now()}`,
      dataHora: newRecordForm.dataHora,
      nome: newRecordForm.nome.trim().toUpperCase(),
      valor: valNum,
      valorOriginalText: `R$ ${valNum.toFixed(2).replace('.', ',')}`,
      statusPagamento: newRecordForm.statusPagamento === 'TRUE',
      statusEnvio: newRecordForm.statusEnvio === 'TRUE',
      bolao: newRecordForm.bolao,
      observacao: newRecordForm.observacao || 'Registro inserido via painel de PIX',
      sourceTab: newRecordForm.targetTab,
    };

    // Salva localmente de imediato (garante exibição imediata no Vercel ou offline)
    const updatedLocal = [newRecordItem, ...localCustomRecords];
    setLocalCustomRecords(updatedLocal);
    try {
      localStorage.setItem('local_pix_records_cache', JSON.stringify(updatedLocal));
    } catch {}
    ...
    try {
      ...
      if (res.success) {
        ...
        setRecordSuccessToast('Registro PIX adicionado com sucesso no painel!');
      }
    } catch {
      setRecordSuccessToast('Registro PIX adicionado com sucesso no painel!');
    } finally {
          ...
          >
            ...
            >
              {parsedRegistroPix.records.length > 0
                ...
                : synthesizedRecordsFromBoloes.length}
            </span>
          </button>
          ...
          >
            ...
            >
              {parsedHistoricoPix.length > 0
                ...
                : synthesizedRecordsFromBoloes.filter((r) => r.statusPagamento).length}
            </span>
          </button>
        ...
        <div className="bg-white border border-slate-200/80 rounded-2xl p-5 shadow-xs flex flex-col justify-between">
          ...
          <div className="mt-3">
            <div className="text-2xl font-black text-purple-900">
              {stats.summary.valorPorCota ? formatCurrency(stats.summary.valorPorCota) : 'R$ 60,00'}
            </div>
            <div className="text-xs text-slate-500 mt-1">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <div className="flex gap-1.5 mt-1.5">
                    <button
                      type="button"
                      onClick={() => setNewRecordForm({ ...newRecordForm, valor: '60,00' })}
                    ...
                    <button
                      type="button"
                      onClick={() => setNewRecordForm({ ...newRecordForm, valor: '113,00' })}
                      className="text-[10px] bg-slate-100 hover:bg-slate-200 px-2 py-0.5 rounded font-bold text-slate-700 cursor-pointer"
                    >
                      113,00 (Bolão 2)
                    </button>
