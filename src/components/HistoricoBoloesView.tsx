...
import { formatCurrency, formatBolaoBetSummary, resolveLotteryType } from '../utils/calculator';
...
export const HistoricoBoloesView: React.FC<HistoricoBoloesProps> = ({
  ...
  const filteredBoloes = boloes.filter((b) => {
    ...
    const resolvedLottery = resolveLotteryType(b.lotteryType, b.title);
    const matchesLottery =
      lotteryFilter === 'todos' ||
      b.lotteryType === lotteryFilter ||
      resolvedLottery === lotteryFilter;
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-100 text-xs">
              {filteredBoloes.map((b) => {
                ...
                const lotKey = resolveLotteryType(b.lotteryType, b.title);
                const lotCfg = LOTTERY_CONFIGS[lotKey];
                    ...
                    <td className="py-4 px-4">
                      <span className="inline-flex items-center gap-1.5 font-bold text-slate-800">
                        <span
                          ...
                          style={{ backgroundColor: lotCfg?.color || '#10b981' }}
                        />
                        <span>{lotCfg?.name || b.lotteryType}</span>
                      </span>
                    </td>
