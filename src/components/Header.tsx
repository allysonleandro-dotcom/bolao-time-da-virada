      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between py-4 gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            {onWriteAllToSheets && (
              <button
                id="header-write-sheets-btn"
                onClick={onWriteAllToSheets}
                disabled={isSyncing}
                title="Gravar todos os bolões e participantes na planilha Google Sheets"
                className="bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl px-3 sm:px-3.5 py-2 text-xs font-black shadow-xs transition flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
              >
                <UploadCloud className={`w-3.5 h-3.5 ${isSyncing ? 'animate-bounce' : ''}`} />
                <span className="hidden sm:inline">Gravar na Planilha</span>
              </button>
            )}
            ...
            {onSyncGoogleSheets && (
              <button
                ...
                className="bg-emerald-50 hover:bg-emerald-100 border border-emerald-300 text-emerald-800 rounded-xl px-3 sm:px-3.5 py-2 text-xs font-black shadow-2xs transition flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
              >
        ...
        <div className="pb-3 pt-1">
          <nav className="flex items-center gap-1.5 bg-slate-100/90 p-1.5 rounded-2xl border border-slate-200/60 overflow-x-auto no-scrollbar max-w-fit">
            ...
            <button
              id="nav-tab-registro-pix"
              onClick={() => setCurrentTab('registro-pix')}
              className={`px-3.5 py-2 rounded-xl text-xs font-bold transition flex items-center gap-1.5 whitespace-nowrap cursor-pointer ${
            ...
            >
              ...
              <span>Gerador de Dezenas</span>
              <span className="bg-emerald-600 text-white text-[9px] px-1.5 py-0.5 rounded-full font-black uppercase">
                PDF
              </span>
            </button>
