...
export const VerificadorLotofacilView: React.FC<VerificadorViewProps> = ({
  ...
  // 2. Modality selection state (Dia de Sorte, Quina, Lotofácil, etc.)
  const [selectedLotteryType, setSelectedLotteryType] = useState<LotteryType>(() => {
    const firstBolao = boloes[0];
    return firstBolao ? resolveLotteryType(firstBolao.lotteryType, firstBolao.title) : 'lotofacil';
  });

  // Standalone tickets when checking without an active bolão
  const [standaloneTickets, setStandaloneTickets] = useState<TicketGame[]>([]);

  // Find active bolão
  const activeBolao = useMemo(() => {
    if (!selectedBolaoId) return null;
    return boloes.find((b) => b.id === selectedBolaoId) || null;
  }, [boloes, selectedBolaoId]);

  // Current lottery type
  const currentLotteryType: LotteryType = activeBolao
    ? resolveLotteryType(activeBolao.lotteryType, activeBolao.title)
    : selectedLotteryType;
  ...
  const standardDrawnCount =
    lotteryConfig.standardBetCount ||
    (currentLotteryType === 'mega-sena'
      ? 6
      : currentLotteryType === 'dia-de-sorte'
      ? 7
      : currentLotteryType === 'quina'
      ? 5
      : 15);
  ...
  const [drawnNumbers, setDrawnNumbers] = useState<number[]>(() => {
    ...
    if (currentLotteryType === 'dia-de-sorte') {
      return [3, 8, 14, 19, 22, 27, 31];
    }
    if (currentLotteryType === 'quina') {
      return [10, 25, 38, 54, 72];
    }
  ...
  // Handler for clicking modality pill
  const handleSelectModality = (lotId: LotteryType) => {
    setSelectedLotteryType(lotId);
    const found = boloes.find(
      (b) => resolveLotteryType(b.lotteryType, b.title) === lotId
    );
    if (found) {
      setSelectedBolaoId(found.id);
      if (onSelectBolao) onSelectBolao(found.id);
    } else {
      setSelectedBolaoId('');
      const defaultDrawn =
        lotId === 'mega-sena'
          ...
          ? [6, 14, 23, 31, 40, 48]
          : lotId === 'lotomania'
          ? [2, 5, 11, 18, 23, 31, 37, 44, 52, 59, 63, 68, 71, 77, 82, 88, 91, 94, 97, 99]
          : [1, 2, 3, 5, 7, 9, 11, 13, 15, 17, 19, 20, 21, 23, 25];
      setDrawnNumbers(defaultDrawn);
      setRealtimeInputText(defaultDrawn.map((n) => String(n).padStart(2, '0')).join(' '));
      setPrizeValues(getDefaultPrizeEstimates(lotId));
      setContestNumberInput('');
      const cfg = LOTTERY_CONFIGS[lotId] || LOTTERY_CONFIGS['lotofacil'];
      setNewTicketNumbersCount(cfg.standardBetCount || 15);
      setNewTicketName(`Jogo 1`);
  ...
  useEffect(() => {
    if (activeBolao) {
      ...
      setPrizeValues(getDefaultPrizeEstimates(currentLotteryType));
  ...
  const handleSaveTicket = () => {
    ...
    const existingTickets = activeBolao?.tickets || standaloneTickets;
    let updatedTickets: TicketGame[] = [...existingTickets];
    ...
    if (activeBolao && onUpdateBolao) {
      ...
      setStandaloneTickets(updatedTickets);
      setSaveSuccessMessage(
        editingTicketId ? '✓ Aposta atualizada com sucesso!' : '✓ Nova aposta adicionada para conferência!'
      );
    }
  ...
  const handleDeleteTicket = (ticketId: string) => {
    if (!confirm('Deseja realmente remover esta aposta?')) return;
    const existingTickets = activeBolao?.tickets || standaloneTickets;
    ...
    if (activeBolao && onUpdateBolao) {
      ...
      setStandaloneTickets(updatedTickets);
      setSaveSuccessMessage('✓ Aposta removida.');
    }
    setTimeout(() => setSaveSuccessMessage(null), 3000);
  };
  ...
  const handleGenerateBatch = () => {
    ...
    const existingTickets = activeBolao?.tickets || standaloneTickets;
    const baseCount = existingTickets.length;
    ...
    if (activeBolao && onUpdateBolao) {
      const updatedBolao: Bolao = {
        ...activeBolao,
        tickets: [...existingTickets, ...newBatch],
      };
      onUpdateBolao(updatedBolao);
      setSaveSuccessMessage(`✓ ${count} apostas geradas e registradas no bolão!`);
    } else {
      setStandaloneTickets([...existingTickets, ...newBatch]);
      setSaveSuccessMessage(`✓ ${count} apostas geradas para conferência!`);
    }
  ...
  // ----------------------------------------------------
  const ticketsToConfer: TicketGame[] = activeBolao?.tickets || standaloneTickets;
  ...
  return (
    <div className="space-y-6 pb-16">
      ...
      <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-2xs space-y-4">
        {/* Lottery Modality Quick Filter Pills */}
        <div className="flex items-center gap-2 overflow-x-auto pb-1 no-scrollbar">
          ...
          {Object.values(LOTTERY_CONFIGS)
            ...
            .map((lot) => {
              const isCurrent = currentLotteryType === lot.id;
              const hasBolao = boloes.some(
                (b) => resolveLotteryType(b.lotteryType, b.title) === lot.id
              );
              return (
                <button
                  key={lot.id}
                  type="button"
                  onClick={() => handleSelectModality(lot.id as LotteryType)}
                  style={{
                    borderColor: isCurrent ? lot.color : undefined,
                    backgroundColor: isCurrent ? `${lot.color}15` : undefined,
                    color: isCurrent ? lot.color : undefined,
                  }}
                  className={`px-3 py-1.5 rounded-xl text-xs font-black border transition flex items-center gap-1.5 shrink-0 cursor-pointer ${
                    isCurrent
        ...
        <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-4 pt-3 border-t border-slate-100">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 flex-1">
            <div className="p-2.5 bg-slate-100 rounded-xl text-slate-800 font-bold text-xs flex items-center gap-1.5 shrink-0">
              ...
              <span>Bolão / Modo:</span>
            </div>
            <select
              value={selectedBolaoId}
              onChange={(e) => {
                const val = e.target.value;
                setSelectedBolaoId(val);
                if (val) {
                  const b = boloes.find((item) => item.id === val);
                  if (b) {
                    const resolved = resolveLotteryType(b.lotteryType, b.title);
                    setSelectedLotteryType(resolved);
                  }
                  if (onSelectBolao) onSelectBolao(val);
                }
              }}
            ...
            >
              <option value="">
                {activeBolao
                  ? '--- Conferência Avulsa / Sem Bolão Vinculado ---'
                  : `(Modo Livre: Conferência Avulsa de ${lotteryConfig.name})`}
              </option>
              {boloes.map((b) => (
                <option key={b.id} value={b.id}>
                  {b.title} (Conc. {b.contestNumber || 'N/A'}) - {LOTTERY_CONFIGS[resolveLotteryType(b.lotteryType, b.title)]?.name || b.lotteryType}
                </option>
              ))}
            </select>
          </div>
