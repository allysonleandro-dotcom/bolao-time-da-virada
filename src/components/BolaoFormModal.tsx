...
import { X, Layers, Calculator, CheckCircle2, RefreshCw, Wand2, AlertTriangle } from 'lucide-react';
import { Bolao, BolaoParticipant, LotteryType, SystemSettings, TicketGame } from '../types';
...
export const BolaoFormModal: React.FC<BolaoFormModalProps> = ({
  ...
  useEffect(() => {
    if (editingBolao) {
      const lot = resolveLotteryType(editingBolao.lotteryType, editingBolao.title);
      const targetConfig = LOTTERY_CONFIGS[lot] || LOTTERY_CONFIGS['mega-sena'];
      const dez =
        editingBolao.dezenas && editingBolao.dezenas >= targetConfig.minNumbers && editingBolao.dezenas <= targetConfig.maxNumbers
          ? editingBolao.dezenas
          : editingBolao.tickets?.[0]?.numbersCount ||
            targetConfig.standardBetCount ||
            6;
      ...
      const totVal = editingBolao.totalBolao && editingBolao.totalBolao > 0
        ? editingBolao.totalBolao
        : Math.round(quotas * qPrice * 100) / 100;
  ...
  const recalculateFromGames = (
    ...
    const roundedQuota = Math.ceil(rawPerQuota * 100) / 100;
  ...
  const valorLiquidoJogos = currentTotalBolao - totalDeducoes;
  const jogosPossiveisCalculados = singleGameCost > 0 ? Math.floor(valorLiquidoJogos / singleGameCost) : 0;
  const totalGastoJogos = gameCount * singleGameCost;
  const saldoFinal = valorLiquidoJogos - totalGastoJogos;
  const isDeficit = saldoFinal < -0.01;
  const sobraOuTroco = Math.max(0, saldoFinal);
  const deficitAmount = Math.max(0, -saldoFinal);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const finalTotalBolao =
      totalBolaoValue > 0
        ? totalBolaoValue
        : Math.round(finalQuotas * finalQuotaPrice * 100) / 100;

    // Pre-generate game slots if new bolão or empty tickets
    ...
    } else if (editingBolao) {
      // If editing existing bolão, update ticket costs, dezenas, and lottery if they were changed
      const config = LOTTERY_CONFIGS[lotteryType] || LOTTERY_CONFIGS['mega-sena'];
      const unitCost = singleGameCost > 0 ? singleGameCost : getOfficialGameCost(lotteryType, dezenas);

      initialTickets = initialTickets.map((t, idx) => {
        let nums = t.numbers ? [...t.numbers] : [];
        if (t.numbersCount !== dezenas || t.lotteryType !== lotteryType || nums.length !== dezenas) {
          const numSet = new Set<number>(nums.filter((n) => n <= config.totalRange));
          while (numSet.size < dezenas) {
            numSet.add(Math.floor(Math.random() * config.totalRange) + 1);
          }
          nums = Array.from(numSet).slice(0, dezenas).sort((a, b) => a - b);
        }
        return {
          ...t,
          lotteryType: lotteryType !== 'combo' ? lotteryType : 'mega-sena',
          ...
          cost: unitCost,
          numbers: nums,
          name: t.name || `Jogo ${idx + 1} (${dezenas} dezenas)`,
        };
      });

      // Synchronize gameCount if changed via byGames
      if (calcBase === 'byGames') {
        if (gameCount > initialTickets.length) {
          while (initialTickets.length < gameCount) {
            const i = initialTickets.length + 1;
            const numbers = new Set<number>();
            while (numbers.size < dezenas) {
              numbers.add(Math.floor(Math.random() * config.totalRange) + 1);
            }
        ...
        } else if (gameCount < initialTickets.length) {
          initialTickets = initialTickets.slice(0, gameCount);
        }
      }
    }

    // Synchronize participants due amounts with new quota price
    let updatedParticipants: BolaoParticipant[] = editingBolao ? [...editingBolao.participants] : [];
    if (editingBolao && updatedParticipants.length > 0) {
      updatedParticipants = updatedParticipants.map((p) => {
        const newDue = Math.round(p.quotas * finalQuotaPrice * 100) / 100;
        let newAmountPaid = p.amountPaid;
        if (p.status === 'pago') {
          newAmountPaid = newDue;
        }
        return {
          ...p,
          totalDue: newDue,
      ...
      <div className="bg-white rounded-3xl max-w-2xl w-full p-6 sm:p-8 shadow-2xl border border-slate-200 space-y-6 my-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-5 text-xs">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-[11px] font-bold text-slate-500">
                Ativa: <strong style={{ color: currentConfig.color }}>{currentConfig.name}</strong>
              </span>
            </div>
              ...
              {Object.values(LOTTERY_CONFIGS).map((lot) => {
                ...
                return (
                  <button
                    ...
                    className={`py-2 px-2.5 text-center rounded-2xl border font-black text-xs transition flex items-center justify-start gap-2 cursor-pointer ${
                      isSelected
                  ...
                  >
                    ...
                    />
                    <span className="whitespace-nowrap font-bold text-xs">{lot.name.replace(' (Multi-Loterias)', '')}</span>
                  </button>
                );
              })}
            </div>
          ...
          <div className="bg-gradient-to-br from-emerald-50 to-teal-50/50 rounded-2xl p-4 border border-emerald-200/80 shadow-2xs space-y-3">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 pt-1">
              ...
              {isDeficit ? (
                <div className="bg-rose-50 p-2.5 rounded-xl border border-rose-200 shadow-2xs">
                  <span className="text-[10px] text-rose-700 font-bold block uppercase">Déficit / Faltam</span>
                ...
                <div className="bg-white/90 p-2.5 rounded-xl border border-emerald-100 shadow-2xs">
                  ...
                  <span className={`text-sm sm:text-base font-black ${sobraOuTroco > 0.01 ? 'text-emerald-700' : 'text-slate-700'}`}>
                    {formatCurrency(sobraOuTroco)}
                  </span>
                  <span className="text-[10px] text-slate-400 font-medium block">
                    {sobraOuTroco > 0.01 ? 'fundo de reserva' : 'valor exato'}
                  </span>
                </div>
              )}
            </div>
            ...
            {isDeficit ? (
              <div className="text-[11px] text-rose-900 bg-rose-50 border border-rose-200 p-2.5 rounded-xl font-medium flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 pt-1">
                <div className="flex items-center gap-1.5">
                  <AlertTriangle className="w-4 h-4 text-rose-600 shrink-0" />
                  <span>
                    Atenção: A arrecadação líquida (<strong>{formatCurrency(valorLiquidoJogos)}</strong>) não cobre{' '}
                    <strong>{gameCount} jogo(s) de {dezenas} dezenas</strong> ({formatCurrency(totalGastoJogos)}). Faltam{' '}
                    <strong className="text-rose-700 font-black">{formatCurrency(deficitAmount)}</strong>.
                  </span>
                </div>
                <button
                  type="button"
                  onClick={handleFitExactGames}
                  className="shrink-0 bg-rose-600 hover:bg-rose-700 text-white font-black text-[11px] px-3 py-1.5 rounded-lg transition active:scale-95 shadow-xs cursor-pointer flex items-center gap-1"
                >
                  <RefreshCw className="w-3 h-3" />
                  <span>Ajustar Cota aos Jogos</span>
                </button>
              </div>
            ) : (
              <div className="text-[11px] text-emerald-900 font-medium flex items-center gap-1.5 pt-1">
                ...
                <span>
                  ...
                  <strong>{totalQuotas} cotas de {formatCurrency(quotaPrice)}</strong> cobre{' '}
                  <strong>{gameCount} jogo(s) de {dezenas} dezenas</strong> ({currentConfig.name})
                  {sobraOuTroco > 0.01 ? (
                    <> com sobra de <strong className="text-emerald-700 font-black">{formatCurrency(sobraOuTroco)}</strong> para fundo de reserva.</>
                  ) : (
                    ' com valor exato.'
                  )}
                </span>
