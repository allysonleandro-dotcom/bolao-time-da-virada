...
export function formatDateLongBR(dateStr?: string): string {
  if (!dateStr) return 'data a ser definida';
  try {
...
export function getLotteryDisplayName(bolao: Bolao): string {
  const titleLower = (bolao.title || '').toLowerCase();
  if (titleLower.includes('independência') || titleLower.includes('independencia')) {
    return 'Lotofácil da Independência';
  }
  if (titleLower.includes('virada')) {
    return 'Mega da Virada';
  }
  if (titleLower.includes('são joão') || titleLower.includes('sao joao')) {
    return 'Quina de São João';
  }
  if (titleLower.includes('primavera')) {
    return 'Lotofácil de Primavera';
  }
  const config = LOTTERY_CONFIGS[bolao.lotteryType];
  return config?.name || bolao.title || 'Loteria';
}

export function generateDefaultStructureText(bolao: Bolao): string {
  if (bolao.tickets && bolao.tickets.length > 0) {
    return formatBolaoBetSummary(bolao, true);
  }
  const dezenas = bolao.dezenas || 18;
  const config = LOTTERY_CONFIGS[bolao.lotteryType] || LOTTERY_CONFIGS['lotofacil'];
  const standard = config.standardBetCount || 15;
  if (dezenas > standard) {
    const equiv = combinations(dezenas, standard);
    return `1 aposta de ${dezenas} dezenas (equivalente a ${equiv} apostas simples)`;
  }
  return `1 aposta de ${dezenas} dezenas`;
}

export function generateDefaultDeclarationText(params: {
  lotteryName: string;
  contestNumber?: string;
  estimatedPrize?: string;
  drawDateFormatted?: string;
}): string {
  const contestPart = params.contestNumber ? ` (Concurso ${params.contestNumber})` : '';
  const prizePart = params.estimatedPrize
    ? `, com prêmio estimado em ${params.estimatedPrize}`
    : '';
  const datePart = params.drawDateFormatted
    ? `, cujo sorteio será realizado no dia ${params.drawDateFormatted}.`
    : '.';

  return `Declaro que recebi minha cota do bolão referente à ${params.lotteryName}${contestPart}${prizePart}${datePart}`;
}
...
export function calculateBolaoFinancials(bolao: Bolao): BolaoFinancials {
  ...
  const totalQuotas = bolao.totalQuotas > 0 ? bolao.totalQuotas : (bolao.totalCotas || 1);
  const suggestedQuotaPrice = Math.ceil((totalBaseExpenses / totalQuotas) * 100) / 100;
  
  const actualQuotaPrice = bolao.quotaPrice || suggestedQuotaPrice;
  const totalExpectedRevenue = bolao.totalBolao && bolao.totalBolao > 0
    ? bolao.totalBolao
    : Math.round(totalQuotas * actualQuotaPrice * 100) / 100;
