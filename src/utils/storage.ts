...
export function loadParticipants(): Participant[] {
  try {
    ...
    if (!raw) {
      const initUppercased = INITIAL_PARTICIPANTS.map((p) => ({
        ...p,
        name: (p.name || '').toUpperCase(),
      }));
      localStorage.setItem(STORAGE_KEYS.PARTICIPANTS, JSON.stringify(initUppercased));
      return initUppercased;
    }
    ...
    const updated = parsed.map((p) => {
      ...
      const name = (init?.name || p.name || '').toUpperCase();
      if (init) {
        return {
          ...p,
    ...
    const missing = INITIAL_PARTICIPANTS.filter((p) => !existingIds.has(p.id)).map((p) => ({
      ...p,
      name: (p.name || '').toUpperCase(),
    }));
    ...
    return INITIAL_PARTICIPANTS.map((p) => ({ ...p, name: (p.name || '').toUpperCase() }));
  }
}

export function saveParticipants(participants: Participant[]): void {
  try {
    const uppercased = participants.map((p) => ({
      ...p,
      name: (p.name || '').toUpperCase(),
    }));
    localStorage.setItem(STORAGE_KEYS.PARTICIPANTS, JSON.stringify(uppercased));
  } catch (err) {
...
export function loadBoloes(): Bolao[] {
  try {
    ...
    const defaultMega1 = INITIAL_BOLOES.find((ib) => ib.id === 'bolao-1-mega-virada-57');
    const defaultMega2 = INITIAL_BOLOES.find((ib) => ib.id === 'bolao-2-mega-virada-35');

    // Sync bolões and migrate legacy bolões if needed
    const updated = parsed.map((b) => {
      // Migrate legacy Bolão 1 if it has not been migrated yet
      if (
        b.id === 'bolao-1-independencia-60' &&
        !parsed.some((other) => other.id === 'bolao-1-mega-virada-57')
      ) {
        if (defaultMega1) {
          return {
            ...defaultMega1,
            participants: b.participants && b.participants.length > 0 ? b.participants : defaultMega1.participants,
          };
        }
      }
      // Migrate legacy Bolão 2 if it has not been migrated yet
      if (
        b.id === 'bolao-2-independencia-39' &&
        !parsed.some((other) => other.id === 'bolao-2-mega-virada-35')
      ) {
        if (defaultMega2) {
          return {
            ...defaultMega2,
            participants: b.participants && b.participants.length > 0 ? b.participants : defaultMega2.participants,
          };
        }
      }

      // Automatically upgrade Bolão 1 if it holds old/prior values (57.12, 50 cotas, or 1 aposta)
      if (
        b.id === 'bolao-1-mega-virada-57' &&
        (b.quotaPrice === 57.12 || b.totalQuotas === 50 || b.title?.includes('1 aposta de 11') || b.quotaPrice !== 60.0)
      ) {
        if (defaultMega1) {
          const parts = (b.participants && b.participants.length > 0 ? b.participants : defaultMega1.participants).map((p) => ({
            ...p,
            amountPaid: p.status === 'pago' ? Math.round((p.quotas || 1) * 60.0 * 100) / 100 : p.amountPaid,
            totalDue: Math.round((p.quotas || 1) * 60.0 * 100) / 100,
          }));
          return {
            ...defaultMega1,
            participants: parts,
      ...
      // Automatically upgrade Bolão 2 if it holds old/prior values (35.70, 40 cotas, or 10 dezenas)
      if (
        b.id === 'bolao-2-mega-virada-35' &&
        (b.quotaPrice === 35.70 || b.totalQuotas === 40 || b.title?.includes('10 dezenas') || b.quotaPrice !== 113.0)
      ) {
        if (defaultMega2) {
          const parts = (b.participants && b.participants.length > 0 ? b.participants : defaultMega2.participants).map((p) => ({
            ...p,
            amountPaid: p.status === 'pago' ? Math.round((p.quotas || 1) * 113.0 * 100) / 100 : p.amountPaid,
            totalDue: Math.round((p.quotas || 1) * 113.0 * 100) / 100,
          }));
          return {
            ...defaultMega2,
            participants: parts,
      ...
      // Automatically upgrade Bolão Regular if it holds old mock values or is missing Cota 1 (Linha 7)
      const hasCota1 = b.participants && b.participants.some((p) => p.quotaNumbers && p.quotaNumbers.includes(1));
      if (
        b.id === 'bolao-regular-lotofacil-25' &&
        (!hasCota1 || b.contestNumber === '3773' || b.title?.includes('TESTE APLICATIVO') || b.totalQuotas === 4 || (b.participants && b.participants.length <= 1))
      ) {
        const defaultReg = INITIAL_BOLOES.find((ib) => ib.id === 'bolao-regular-lotofacil-25');
        if (defaultReg) {
          return {
            ...defaultReg,
          };
      ...
      // Ensure totalBolao exists and is consistent with quotas and quotaPrice
      const quotas = b.totalQuotas || b.totalCotas || 1;
      const qPrice = b.quotaPrice || 10;
      const effectiveTotalBolao =
        b.totalBolao && b.totalBolao > 0
          ? b.totalBolao
          : Math.round(quotas * qPrice * 100) / 100;

      return {
        ...b,
        totalBolao: effectiveTotalBolao,
      };
    });

    // Ensure Mega da Virada bolões are always present
    const hasMega1 = updated.some((b) => b.id === 'bolao-1-mega-virada-57');
    const hasMega2 = updated.some((b) => b.id === 'bolao-2-mega-virada-35');
    if (!hasMega1 && defaultMega1) updated.push(defaultMega1);
    if (!hasMega2 && defaultMega2) updated.push(defaultMega2);
