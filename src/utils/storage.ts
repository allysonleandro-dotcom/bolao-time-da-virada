...
export function loadBoloes(): Bolao[] {
  try {
    ...
    const updated = parsed.map((b) => {
      ...
      // Automatically upgrade Bolão Regular if it holds old mock values (concurso 3773, 4 cotas, or old participant count)
      if (
        b.id === 'bolao-regular-lotofacil-25' &&
        (b.contestNumber === '3773' || b.title?.includes('TESTE APLICATIVO') || b.totalQuotas === 4 || (b.participants && b.participants.length <= 1))
      ) {
