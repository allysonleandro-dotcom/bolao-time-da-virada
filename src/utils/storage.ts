...
export function loadBoloes(): Bolao[] {
  try {
    ...
    const updated = parsed.map((b) => {
      ...
      // Automatically upgrade Bolão Regular if it holds old mock values (concurso 3773 or 50 cotas or TESTE APLICATIVO)
      if (
        b.id === 'bolao-regular-lotofacil-25' &&
        (b.contestNumber === '3773' || b.title?.includes('TESTE APLICATIVO') || (b.totalQuotas === 50 && !b.title?.includes('3788')))
      ) {
