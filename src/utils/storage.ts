...
export function loadBoloes(): Bolao[] {
  try {
    ...
    // Sync bolões with current Mega da Virada definitions
    const updated = parsed.map((b) => {
      if (
        b.id === 'bolao-1-mega-virada-57' ||
        (b.id === 'bolao-1-independencia-60' && !parsed.some((other) => other.id === 'bolao-1-mega-virada-57')) ||
        b.title?.toLowerCase().includes('mega da virada 1') ||
        b.title?.toLowerCase().includes('bolão 1 da mega da virada')
      ) {
        if (defaultMega1) {
          return {
            ...b,
            id: 'bolao-1-mega-virada-57',
            title: defaultMega1.title,
            lotteryType: defaultMega1.lotteryType,
            contestNumber: defaultMega1.contestNumber,
            drawDate: defaultMega1.drawDate,
            totalQuotas: 50,
            totalCotas: 50,
            dezenas: 11,
            quotaPrice: 57.12,
            reserveFundAmount: 84.0,
            notes: defaultMega1.notes,
            tickets: defaultMega1.tickets,
      ...
      if (
        ...
        (b.id === 'bolao-2-independencia-39' && !parsed.some((other) => other.id === 'bolao-2-mega-virada-35')) ||
        b.title?.toLowerCase().includes('mega da virada 2') ||
        b.title?.toLowerCase().includes('bolão 2 da mega da virada')
      ) {
        if (defaultMega2) {
          return {
            ...b,
            id: 'bolao-2-mega-virada-35',
            title: defaultMega2.title,
            lotteryType: defaultMega2.lotteryType,
            contestNumber: defaultMega2.contestNumber,
            drawDate: defaultMega2.drawDate,
            totalQuotas: 40,
            totalCotas: 40,
            dezenas: 10,
            quotaPrice: 35.70,
            reserveFundAmount: 168.0,
            notes: defaultMega2.notes,
            tickets: defaultMega2.tickets,
