...
export function exportBolaoQuotaReceiptsPDF(options: ExportQuotaReceiptsOptions) {
  ...
  autoTable(doc, {
    startY: currentY,
    head: [['Cota', 'Participante', 'Contato / Telefone', 'Valor Cota', 'Assinatura do Participante']],
    body: tableRows,
    theme: 'grid',
    headStyles: {
    ...
    didDrawCell: (data) => {
      // Draw a neat signature line in the signature column
      if (data.section === 'body' && data.column.index === 4) {
        const x1 = data.cell.x + 3;
        const x2 = data.cell.x + data.cell.width - 3;
        const y = data.cell.y + data.cell.height - 3.2;
