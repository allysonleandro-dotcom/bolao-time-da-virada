...
export interface Bolao {
  ...
  title: string;
  lotteryType: LotteryType;
  comboLotteries?: LotteryType[];
  contestNumber: string; // e.g. "2780"
  drawDate: string; // YYYY-MM-DD
  totalQuotas: number;
  totalCotas?: number; // Total de Cotas (suporte a ambas as nomenclaturas)
  totalBolao?: number; // Valor Total do Bolão (arrecadação total esperada)
  quotaPrice: number;
  dezenas?: number; // Quantidade de dezenas por aposta/jogo
...
export interface PixTransactionRecord {
  ...
  nome: string;
  valor: number;
  valorOriginalText: string;
  statusPagamento: boolean; // TRUE / FALSE
  statusEnvio?: boolean; // Transferência Nubank TRUE / FALSE
  numeroCota?: string;
  bolao?: string;
  observacao?: string;
  sourceTab: 'REGISTRO_PIX' | 'HISTORICO_PIX' | 'PIX_PESSOAL';
}

export interface PixSummaryData {
  caixa?: number;
  falta?: number | string;
  custoBolao?: number | string;
  totalCotas?: number;
