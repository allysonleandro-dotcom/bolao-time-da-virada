...
import { GeradorDezenasView } from './components/GeradorDezenasView';
import { RegistroPixView } from './components/RegistroPixView';
...
export default function App() {
  ...
  // Gravação explícita de todos os dados locais na planilha Google Sheets (Push)
  const handleWriteAllToSheets = async (customUrl?: string) => {
    setIsSyncingSheets(true);
    const targetUrl = customUrl || settings.googleSheetsUrl || DEFAULT_SHEET_URL;
    try {
      const res = await triggerWriteAllToGoogleSheets(boloes, participants, targetUrl);
      if (res.success) {
        ...
        setSyncToast({
          message: `Gravação concluída! ${res.syncedBoloesCount} bolão(ões) e ${res.syncedContactsCount} contatos gravados no Google Sheets.`,
          type: 'success',
        });
      } else {
        setSyncToast({
          message: res.message || 'Erro ao gravar dados na planilha. Verifique se o e-mail do sistema tem permissão de Editor.',
          type: 'error',
        });
      }
    } catch (err: any) {
      ...
      setSyncToast({
        message: err?.message || 'Falha ao salvar dados no Google Sheets.',
        type: 'error',
      });
    } finally {
      setIsSyncingSheets(false);
  ...
  // Manual Google Sheets sync handler (Prioriza leitura da planilha para preservar edições feitas na planilha)
  const handleSyncGoogleSheets = async (customUrl?: string) => {
    setIsSyncingSheets(true);
    const targetUrl = customUrl || settings.googleSheetsUrl || DEFAULT_SHEET_URL;
    const spreadsheetId = extractSpreadsheetId(targetUrl);
    try {
      // 1. Lê primeiro os dados mais recentes da planilha (evita sobrescrever edições feitas na planilha)
      const result = await syncFromGoogleSheets(targetUrl, boloes, participants);
      let updatedBoloes = boloes;
      let updatedParticipants = participants;

      if (result.success) {
        updatedBoloes = result.boloes;
        updatedParticipants = result.participants;
        setBoloes(result.boloes);
        setParticipants(result.participants);
      }

      // 2. Sincroniza novos contatos na aba CONTATO se existirem
      for (const p of updatedParticipants.slice(0, 30)) {
        try {
          await triggerSyncContactInSheets(p.name, p.phone || '', spreadsheetId);
        } catch (e) {
      ...
      setSyncToast({
        message: `Planilha Google Sheets sincronizada com sucesso! Dados atualizados.`,
        type: 'success',
      });
    } catch (err: any) {
      console.error('Erro na sincronização Google Sheets:', err);
      setSyncToast({ message: 'Erro ao conectar ou sincronizar com Google Sheets', type: 'error' });
    } finally {
      setIsSyncingSheets(false);
  ...
  // Bolão actions
  const handleCreateOrUpdateBolao = async (bolao: Bolao) => {
    ...
    // Sincronismo simultâneo em tempo real com o Google Sheets ao abrir/atualizar bolão
    const spreadsheetId = extractSpreadsheetId(settings.googleSheetsUrl);
    setSyncToast({
      message: `Salvando e sincronizando "${bolao.title}" na planilha Google Sheets...`,
      type: 'success',
    });
    try {
      if (!exists) {
      ...
      const syncRes = await triggerFullSyncBolaoInSheets(bolao, participants, spreadsheetId);
      if (syncRes.success) {
        setSyncToast({
          message: `"${bolao.title}" gravado com sucesso na planilha Google Sheets!`,
          type: 'success',
        });
      } else {
        setSyncToast({
          message: `Bolão salvo localmente. Atenção ao gravar na planilha: ${syncRes.error || 'Verifique se a conta de serviço é Editor da planilha.'}`,
          type: 'error',
        });
      }
    } catch (err: any) {
      console.warn('Sincronismo em background Google Sheets:', err);
      setSyncToast({
        message: `Salvo localmente. Erro ao conectar com Google Sheets: ${err?.message || ''}`,
        type: 'error',
      });
    } finally {
      setTimeout(() => setSyncToast(null), 5000);
  ...
  const handleAddParticipant = async (participant: Participant) => {
    ...
    const spreadsheetId = extractSpreadsheetId(settings.googleSheetsUrl);
    triggerSyncContactInSheets(participant.name, participant.phone || '', spreadsheetId).catch((err) =>
  ...
  // Generator transfer with pre-generated tickets
  const handleCreateBolaoFromGeneratedTickets = (data: {
    ...
    setSimulationPreset({
      ...
      adminFeePercent: settings.defaultAdminFeePercent || 0,
      extraCost: 0,
      notes: `Palpites gerados via Gerador de Dezenas (${data.tickets.length} apostas)`,
      tickets: data.tickets.map((t) => ({
        lotteryType: t.lotteryType || data.lotteryType,
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-900 flex flex-col font-sans antialiased">
      ...
      <Header
        ...
        onWriteAllToSheets={() => handleWriteAllToSheets()}
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 pt-6">
        {selectedBolao ? (
          ...
          <BolaoDetailView
            ...
            spreadsheetUrl={settings.googleSheetsUrl}
            onSyncGoogleSheets={handleSyncGoogleSheets}
          />
        ) : (
          ...
          <>
            ...
            {currentTab === 'gerador' && (
              <GeradorDezenasView
                onCreateBolaoWithTickets={handleCreateBolaoFromGeneratedTickets}
              />
            )}
