...
export default function App() {
  ...
  const handleSyncGoogleSheets = async (customUrl?: string) => {
    ...
    try {
      ...
      for (const b of boloes) {
        try {
          await triggerFullSyncBolaoInSheets(b, participants, spreadsheetId);
          syncedCount++;
        } catch (e) {
      ...
      setSyncToast({
        message: `Planilha Google Sheets sincronizada com sucesso! ${syncedCount} bolão(ões) e contatos atualizados.`,
        type: 'success',
      });
    } catch (err: any) {
  ...
  const handleCreateOrUpdateBolao = async (bolao: Bolao) => {
    ...
    try {
      if (!exists) {
        await triggerCreateBolaoTabInSheets(bolao.title, spreadsheetId);
      }
      await triggerFullSyncBolaoInSheets(bolao, participants, spreadsheetId);
      setSyncToast({
        message: `"${bolao.title}" sincronizado com sucesso na planilha Google Sheets!`,
        type: 'success',
      });
    } catch (err) {
