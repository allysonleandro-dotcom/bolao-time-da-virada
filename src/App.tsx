...
export default function App() {
  ...
  // Manual Google Sheets sync handler (Bidirecional: grava dados locais e consolida com a planilha)
  const handleSyncGoogleSheets = async (customUrl?: string) => {
    setIsSyncingSheets(true);
    const targetUrl = customUrl || settings.googleSheetsUrl || DEFAULT_SHEET_URL;
    const spreadsheetId = extractSpreadsheetId(targetUrl);
    try {
      // 1. Grava primeiro todos os bolões ativos e participantes locais nas abas da planilha
      let syncedCount = 0;
      for (const b of boloes) {
        try {
          const res = await triggerFullSyncBolaoInSheets(b, participants, spreadsheetId);
          if (res.success) syncedCount++;
        } catch (e) {
          console.warn('Erro ao enviar bolão para a planilha:', b.title, e);
        }
      }

      // 2. Sincroniza contatos na aba CONTATO
      ...
      // 3. Lê os dados consolidados da planilha
      ...
      setSyncToast({
        message: `Planilha Google Sheets sincronizada com sucesso! ${syncedCount} bolão(ões) atualizados.`,
        type: 'success',
      });
    } catch (err: any) {
      console.error('Erro na sincronização Google Sheets:', err);
      setSyncToast({ message: 'Erro ao conectar ou gravar no Google Sheets', type: 'error' });
    } finally {
      setIsSyncingSheets(false);
