...
export default function App() {
  ...
  const handleCreateOrUpdateBolao = async (bolao: Bolao) => {
    ...
    if (!exists) {
      setSyncToast({
        message: `Criando aba "${bolao.title}" na planilha Google Sheets...`,
        type: 'success',
      });
      try {
        const tabRes = await triggerCreateBolaoTabInSheets(bolao.title, spreadsheetId);
        if (tabRes.success) {
          if (bolao.participants && bolao.participants.length > 0) {
            await triggerFullSyncBolaoInSheets(bolao, participants, spreadsheetId);
          }
          setSyncToast({
            message: `Nova aba "${bolao.title}" criada e sincronizada com sucesso na planilha!`,
            type: 'success',
          });
        } else {
          setSyncToast({
            message: `Bolão salvo no app! Aba "${bolao.title}" pronta para envio no Google Sheets.`,
            type: 'success',
          });
        }
      } catch (err) {
