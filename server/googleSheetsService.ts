...
export function identifyBolaoType(
  ...
  if (
    ...
    normId.includes('regular') ||
    normTitle === 'BOLOES_REGULARES' ||
    normTitle === 'BOLOES REGULARES' ||
    normTitle === 'BOLAO REGULAR' ||
    normTitle.includes('REGULAR') ||
    normTitle.includes('REGULARES') ||
    normTitle.includes('3788') ||
    normTitle.includes('3773') ||
    (normTitle.includes('LOTOFACIL') && !normTitle.includes('INDEPENDENCIA') && !normTitle.includes('VIRADA'))
  ) {
    return 'regular';
  }
...
/**
 * Obtém o nome real da aba de Bolões Regulares presente na planilha
 */
export async function getBoloesRegularesTabTitle(sheets: any, spreadsheetId: string): Promise<string> {
  try {
    const meta = await sheets.spreadsheets.get({ spreadsheetId });
    const tabList = (meta?.data?.sheets || []).map((s: any) => s.properties?.title || '');
    const found = tabList.find((t: string) => {
      const u = normalizeStr(t).replace(/[\s_-]+/g, '');
      return u.includes('BOLOESREGULARES') || u.includes('BOLAOREGULAR') || u === 'REGULARES';
    });
    if (found) return found;
  } catch {}
  return 'BOLÕES_REGULARES';
}
  ...
  try {
    ...
    if (bolaoType === 'mega_virada_1' || bolaoType === 'independencia_1') {
      ...
      const readRes = await sheets.spreadsheets.values.get({
        spreadsheetId: cleanSpreadsheetId,
        range: `'${viradaTab}'!A8:D107`,
      });
      const rows = readRes.data.values || [];
      const searchNorm = normalizeStr(cleanName);
      let targetRowIndexes: number[] = [];

      // 1. Busca por cotas específicas se fornecidas (1 a 100)
      ...
      // 3. Primeira cota vazia se não foi encontrado (até no máximo linha 107)
      ...
      // Se ainda não achou e não atingiu o limite de 100 cotas
      if (targetRowIndexes.length === 0 && rows.length < maxSlots) {
        targetRowIndexes.push(8 + rows.length);
      }

      if (targetRowIndexes.length === 0) {
        throw new Error('Todas as 100 cotas do Bolão 1 já estão preenchidas.');
      }

      const defaultPrice = 60.00;
      ...
      for (const targetRowIndex of targetRowIndexes) {
        if (targetRowIndex > 107) continue; // Nunca ultrapassar a linha 107 (Cota 100)
        const cotaNumber = targetRowIndex - 7;
        const cotaStr = `${cotaNumber}.`;

        // Grava na aba da Mega da Virada (A8:D107)
        await sheets.spreadsheets.values.update({
          spreadsheetId: cleanSpreadsheetId,
        ...
        if (isPaid) {
          await registrarTransacaoPix(
            cleanSpreadsheetId,
            cleanName,
            dados.valor ? `R$ ${dados.valor.toFixed(2)}` : 'R$ 60,00',
            cotaNumber.toString(),
    ...
    if (bolaoType === 'mega_virada_2' || bolaoType === 'independencia_2') {
      const maxSlots = 100; // Bolão 2 possui até 100 cotas (linhas 8 a 107)
      const viradaTab = await getViradaTabTitle(sheets, cleanSpreadsheetId);
      const readRes = await sheets.spreadsheets.values.get({
        spreadsheetId: cleanSpreadsheetId,
        range: `'${viradaTab}'!F8:I107`,
      });
      const rows = readRes.data.values || [];
      const searchNorm = normalizeStr(cleanName);
      let targetRowIndexes: number[] = [];

      // 1. Busca por cotas específicas (1 a 100)
      if (dados.cotaNumbers && dados.cotaNumbers.length > 0) {
        targetRowIndexes = dados.cotaNumbers
          .map((cota) => Number(cota))
          .filter((cota) => !isNaN(cota) && cota >= 1 && cota <= maxSlots)
      ...
      // 2. Busca por nome dentro das 100 cotas
      if (targetRowIndexes.length === 0 && searchNorm) {
        for (let i = 0; i < Math.min(rows.length, maxSlots); i++) {
          const rowName = normalizeStr(rows[i][1] || '');
          if (rowName && (rowName === searchNorm || rowName.includes(searchNorm) || searchNorm.includes(rowName))) {
      ...
      // 3. Primeira cota vazia dentro das 100 cotas (linhas 8 a 107)
      if (targetRowIndexes.length === 0) {
        const quotasRequested = Math.max(1, Math.round(dados.quotas || 1));
        for (let i = 0; i < maxSlots; i++) {
          const rowName = (rows[i]?.[1] || '').trim();
      ...
      // Se ainda não achou e não atingiu 100 cotas
      if (targetRowIndexes.length === 0 && rows.length < maxSlots) {
        targetRowIndexes.push(8 + rows.length);
      }

      if (targetRowIndexes.length === 0) {
        throw new Error('Todas as 100 cotas do Bolão 2 já estão preenchidas.');
      }

      const defaultPrice = 113.00;
      const valorNum = isPaid ? (dados.valor ? (parseMoneyValue(dados.valor) || defaultPrice) : defaultPrice) : 0;

      for (const targetRowIndex of targetRowIndexes) {
        if (targetRowIndex > 107) continue; // NUNCA ultrapassar a linha 107 (Cota 100 do Bolão 2)
        const cotaNumber = targetRowIndex - 7;
        const cotaStr = `${cotaNumber}.`;

        // Grava na aba da Mega da Virada (F8:I107)
        await sheets.spreadsheets.values.update({
          spreadsheetId: cleanSpreadsheetId,
        ...
        if (isPaid) {
          await registrarTransacaoPix(
            cleanSpreadsheetId,
            cleanName,
            dados.valor ? `R$ ${dados.valor.toFixed(2)}` : 'R$ 113,00',
            cotaNumber.toString(),
    ...
    if (bolaoType === 'regular') {
      const regTab = await getBoloesRegularesTabTitle(sheets, spreadsheetId);
      const readRes = await sheets.spreadsheets.values.get({
        spreadsheetId,
        range: `'${regTab}'!A7:D120`,
      });
      const rows = readRes.data.values || [];
      const searchNorm = normalizeStr(cleanName);
      let targetRowIndexes: number[] = [];
      ...
      for (const targetRowIndex of targetRowIndexes) {
        ...
        await sheets.spreadsheets.values.update({
          spreadsheetId,
          range: `'${regTab}'!A${targetRowIndex}:D${targetRowIndex}`,
          valueInputOption: 'USER_ENTERED',
          requestBody: {
      ...
      return {
        ...
        action: 'updated',
        rowNumber: targetRowIndexes[0],
        message: `Participante "${cleanName}" atualizado na aba "${regTab}" (${targetRowIndexes.length} cota(s))!`,
      };
    }
...
export async function removerParticipanteDoBolao(
  ...
  try {
    ...
    if (bolaoType === 'regular') {
      const regTab = await getBoloesRegularesTabTitle(sheets, cleanSpreadsheetId);
      const readRes = await sheets.spreadsheets.values.get({
        spreadsheetId: cleanSpreadsheetId,
        range: `'${regTab}'!A7:D120`,
      });
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        if (rowName && searchNorm && (rowName === searchNorm || rowName.includes(searchNorm) || searchNorm.includes(rowName))) {
          ...
          await sheets.spreadsheets.values.update({
            spreadsheetId: cleanSpreadsheetId,
            range: `'${regTab}'!A${targetRow}:D${targetRow}`,
            valueInputOption: 'USER_ENTERED',
            requestBody: {
      ...
      if (removedCount > 0) {
        return {
          success: true,
          message: `Participante "${cleanName}" removido de ${removedCount} cota(s) na aba "${regTab}"!`,
        };
      }
    }
...
export async function sincronizarBolaoCompleto(
  ...
  try {
    ...
    // 1. BOLÃO 1 DA MEGA DA VIRADA / INDEPENDÊNCIA -> Gravação atômica em lote em A8:D107
    if (bolaoType === 'mega_virada_1' || bolaoType === 'independencia_1') {
      const viradaTab = await getViradaTabTitle(sheets, cleanSpreadsheetId);
      const totalSlots = Math.max(100, bolao.totalQuotas || 100);
      const admRows: (string | number)[][] = [];
      const partRows: string[][] = [];
      const quotaVal = bolao.quotaPrice || 60.0;

      // Lê o estado atual da planilha para preservar cotas existentes caso não venham no envio
      const readRes = await sheets.spreadsheets.values.get({
        spreadsheetId: cleanSpreadsheetId,
        range: `'${viradaTab}'!A8:D${7 + totalSlots}`,
      }).catch(() => null);
      const existingRows = readRes?.data?.values || [];
      ...
      // Atualiza na aba da Mega da Virada
      await sheets.spreadsheets.values.update({
        spreadsheetId: cleanSpreadsheetId,
        range: `'${viradaTab}'!A8:D${7 + totalSlots}`,
        valueInputOption: 'USER_ENTERED',
      ...
      await sheets.spreadsheets.values.update({
        spreadsheetId: cleanSpreadsheetId,
        range: `'PARTICIPANTES INDEPENDÊNCIA'!A5:C${4 + totalSlots}`,
        valueInputOption: 'USER_ENTERED',
    ...
    // 2. BOLÃO 2 DA MEGA DA VIRADA / INDEPENDÊNCIA -> Gravação atômica em lote em F8:I107
    if (bolaoType === 'mega_virada_2' || bolaoType === 'independencia_2') {
      const viradaTab = await getViradaTabTitle(sheets, cleanSpreadsheetId);
      const totalSlots = Math.max(100, bolao.totalQuotas || 100);
      const admRows: (string | number)[][] = [];
      const partRows: string[][] = [];
      const quotaVal = bolao.quotaPrice || 113.0;

      // Lê o estado atual da planilha para preservar cotas existentes do Bolão 2
      const readRes = await sheets.spreadsheets.values.get({
        spreadsheetId: cleanSpreadsheetId,
        range: `'${viradaTab}'!F8:I${7 + totalSlots}`,
      }).catch(() => null);
      const existingRows = readRes?.data?.values || [];
      ...
      // Atualiza na aba da Mega da Virada (F8:I)
      await sheets.spreadsheets.values.update({
        spreadsheetId: cleanSpreadsheetId,
        range: `'${viradaTab}'!F8:I${7 + totalSlots}`,
        valueInputOption: 'USER_ENTERED',
      ...
      await sheets.spreadsheets.values.update({
        spreadsheetId: cleanSpreadsheetId,
        range: `'PARTICIPANTES INDEPENDÊNCIA'!E5:G${4 + totalSlots}`,
        valueInputOption: 'USER_ENTERED',
        requestBody: { values: partRows },
      }).catch(() => null);
    ...
    if (bolaoType === 'regular') {
      const regTab = await getBoloesRegularesTabTitle(sheets, cleanSpreadsheetId);
      const totalSlots = bolao.totalQuotas && bolao.totalQuotas > 0 ? bolao.totalQuotas : 4;
      ...
      return {
        success: true,
        message: `Bolão Regular sincronizado com sucesso (${bolao.participants.length} participantes, ${totalSlots} cotas na aba "${regTab}")!`,
        totalSynced: bolao.participants.length,
      };
    }
...
export async function lerDadosCompletosPlanilha(
  ...
  try {
    ...
    const findTab = (nameOrFragment: string) => {
      const cleanSearch = normalizeStr(nameOrFragment).replace(/[\s_-]+/g, '');
      return tabs.find((t) => {
        const cleanTab = normalizeStr(t.title).replace(/[\s_-]+/g, '');
        return cleanTab.includes(cleanSearch) || cleanSearch.includes(cleanTab);
      })?.title;
    };
    ...
    const boloesRegularesTitle = findTab('BOLÕES_REGULARES') || findTab('BOLOES_REGULARES') || findTab('REGULARES');
    ...
    const registroPixTitle =
      ...
      findTab('REGISTRO DE PIX') ||
      findTab('REGISTRO PIX');
    const historicoPixTitle =
      findTab('HISTÓRICO DE TRANSAÇÕES PIX') ||
      findTab('HISTORICO DE TRANSACOES PIX') ||
      findTab('HISTÓRICO PIX');
    const pixPessoalTitle =
      findTab('PIX_PESSOAL') ||
      findTab('PIX PESSOAL');
    ...
    if (boloesRegularesTitle) {
      rangeRequests.push({ key: 'boloesRegulares', range: `'${boloesRegularesTitle}'!A1:J150`, title: boloesRegularesTitle });
    }
    if (contatosTitle) {
    ...
    if (registroPixTitle) {
      rangeRequests.push({ key: 'registroPix', range: `'${registroPixTitle}'!A1:N250`, title: registroPixTitle });
    }
    if (historicoPixTitle) {
      rangeRequests.push({ key: 'historicoPix', range: `'${historicoPixTitle}'!A1:N250`, title: historicoPixTitle });
    }
    if (pixPessoalTitle) {
      rangeRequests.push({ key: 'pixPessoal', range: `'${pixPessoalTitle}'!A1:N250`, title: pixPessoalTitle });
    }
    ...
    let historicoPix: string[][] = [];
    let pixPessoal: string[][] = [];
    ...
    if (allRanges.length > 0) {
      ...
      rangeRequests.forEach((reqItem, idx) => {
        ...
        else if (reqItem.key === 'historicoPix') historicoPix = rows;
        else if (reqItem.key === 'pixPessoal') pixPessoal = rows;
...
/**
 * 10. ADICIONAR REGISTRO NA ABA REGISTRO_PIX OU HISTÓRICO PIX
 */
export async function adicionarRegistroPix(
  spreadsheetId: string,
  record: {
    dataHora?: string;
    nome: string;
  ...
  try {
    ...
    // Obtém metadados para achar o nome exato da aba
    const metaRes = await sheets.spreadsheets.get({ spreadsheetId: cleanSpreadsheetId });
    const tabTitles = (metaRes.data.sheets || []).map((s: any) => s.properties?.title || '');

    const findTab = (name: string) =>
      tabTitles.find((t: string) => t.trim().toUpperCase().includes(name.toUpperCase()));

    let tabName = '';
    const nowStr = record.dataHora || new Date().toLocaleString('pt-BR');
    const valorFormatted =
      typeof record.valor === 'number'
        ? `R$ ${record.valor.toFixed(2).replace('.', ',')}`
        : String(record.valor);

    if (record.targetTab === 'HISTORICO_PIX') {
      tabName =
        findTab('HISTÓRICO DE TRANSAÇÕES PIX') ||
        findTab('HISTORICO DE TRANSACOES PIX') ||
        'HISTÓRICO DE TRANSAÇÕES PIX';
      await sheets.spreadsheets.values.append({
        ...
        insertDataOption: 'INSERT_ROWS',
        requestBody: {
          values: [[nowStr, record.nome, valorFormatted, record.statusPagamento || 'TRUE']],
        },
      });
    } else if (record.targetTab === 'PIX_PESSOAL') {
      tabName = findTab('PIX_PESSOAL') || findTab('PIX PESSOAL') || 'PIX_PESSOAL';
      await sheets.spreadsheets.values.append({
        spreadsheetId: cleanSpreadsheetId,
        range: `'${tabName}'!A:E`,
        valueInputOption: 'USER_ENTERED',
      ...
      tabName = findTab('REGISTRO_PIX') || findTab('REGISTRO DE PIX') || 'REGISTRO_PIX';
      await sheets.spreadsheets.values.append({
        spreadsheetId: cleanSpreadsheetId,
        range: `'${tabName}'!A:G`,
        valueInputOption: 'USER_ENTERED',
        ...
        requestBody: {
          values: [
            [
              ...
              valorFormatted,
              record.statusPagamento || 'TRUE',
              record.statusEnvio || 'TRUE',
              record.numeroCota ? String(record.numeroCota) : '',
              record.bolao || 'Bolão 1 da Mega da Virada',
            ],
          ],
        },
      });
    ...
    return {
      success: true,
      message: `Registro PIX gravado com sucesso na aba "${tabName}".`,
    };
  } catch (err: any) {
    console.error('Erro em adicionarRegistroPix:', err);
    throw new Error(`Falha ao adicionar registro PIX: ${err?.message || err}`);
  }
}
