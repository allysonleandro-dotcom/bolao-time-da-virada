...
export async function registrarTransacaoPix(
  ...
  try {
    ...
    const metaRes = await sheets.spreadsheets.get({ spreadsheetId }).catch(() => null);
...
export async function lerDadosCompletosPlanilha(
  ...
  try {
    ...
    const admIndependenciaTitle = findTab('ADM.INDEPENDÊNCIA') || findTab('INDEPENDÊNCIA');
    ...
    if (allRanges.length > 0) {
      ...
      rangeRequests.forEach((reqItem, idx) => {
        ...
        if (reqItem.key === 'admIndependencia') admIndependencia = rows;
